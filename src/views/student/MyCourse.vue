...
export default {
  methods: {
    ...
    yes(){
      ...
      axios.post('http://localhost:8181/mooc/course/delete/'+this.tmp_id+'/'+this.id).then(resp => {
        if(resp.data === 'failed'){
          this.$message({
            message: '操作失败，请稍后再试！',
            type: 'error'
  ...
  created() {
    ...
    axios.get('http://localhost:8181/mooc/course/myCourse/'+this.$route.query.id).then(
      function (resp){
